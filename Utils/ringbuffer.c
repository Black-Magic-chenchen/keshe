...
int32_t ICACHE_FLASH_ATTR rbCanRead(rb_t *rb)
{
    ...
    return rbCapacity(rb) - (rb->rbReadIndex - rb->rbWriteIndex);
}
...
int32_t ICACHE_FLASH_ATTR rbRead(rb_t *rb, void *data, size_t count)
{
    ...
    if (rb->rbReadIndex < rb->rbWriteIndex)
    {
        ...
        memcpy(data,&rb->rbBuff[rb->rbReadIndex], copySz);
        rb->rbReadIndex += copySz;
        return copySz;
    }
    else
    {
        if (count < rbCapacity(rb)-rb->rbReadIndex)
        {
            copySz = count;
            memcpy(data, &rb->rbBuff[rb->rbReadIndex], copySz);
            rb->rbReadIndex += copySz;
        ...
        {
            copySz = rbCapacity(rb) - rb->rbReadIndex;
            memcpy(data, &rb->rbBuff[rb->rbReadIndex], copySz);
            rb->rbReadIndex = 0;
...
int32_t ICACHE_FLASH_ATTR rbWrite(rb_t *rb, const void *data, size_t count)
{
    ...
    if (rb->rbReadIndex <= rb->rbWriteIndex)
    {
        tailAvailSz = rbCapacity(rb) - rb->rbWriteIndex;
        if (count <= tailAvailSz)
        {
            memcpy(&rb->rbBuff[rb->rbWriteIndex], data, count);
            rb->rbWriteIndex += count;
            if (rb->rbWriteIndex >= rbCapacity(rb))
            {
                rb->rbWriteIndex = 0;
            }
            return count;
        ...
        {
            memcpy(&rb->rbBuff[rb->rbWriteIndex], data, tailAvailSz);
            rb->rbWriteIndex = 0;
